...
deploy_project_code() {
    ...
    if [ -f package.json ]; then
        ...
        if npm run | grep -q "build"; then
            ...
            # Ensure dist directory exists and copy built files
            if [ -d "dist" ]; then
                log "INFO" "Copying built files to dist directory..."
                mkdir -p "$DIST_DIR"
                cp -r dist/* "$DIST_DIR/"
            fi
        fi
...
create_sitemap_placeholders() {
    ...
    cat > "$DIST_DIR/sitemap-index.xml" << 'SITEMAP_EOF'
...
SITEMAP_EOF
    ...
    cat > "$DIST_DIR/sitemap.xml" << 'MAIN_SITEMAP_EOF'
<?xml version="1.0" encoding="UTF-8"?>
...
main() {
    ...
    find "$DIST_DIR" -name "sitemap*.xml" -exec chmod 644 {} \; 2>/dev/null || true
    chmod 644 "$DIST_DIR/manifest.json"
