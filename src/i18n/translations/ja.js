/**
 * Japanese (日本語) Translations
 * Complete translation file for the Decision Wheel Spinner application
 * Includes all UI texts, messages, and SEO content
 */

export default {
  // Application meta information for SEO
  meta: {
    title: '決定ルーレット - 簡単に決断を下す',
    description: '美しい決定ルーレットで簡単に決断を下しましょう。選択肢を追加してルーレットを回し、ランダムな結果を得られます。',
    keywords: 'ルーレット, 決定, ランダム, 選択, オプション, 回転, 決める',
    ogTitle: '決定ルーレット - 簡単に決断を下す',
    ogDescription: '美しい決定ルーレットで簡単に決断を下しましょう。決められない時に最適です！',
    twitterTitle: '決定ルーレット - 簡単に決断を下す',
    twitterDescription: '美しい決定ルーレットで簡単に決断を下しましょう。'
  },

  // Main application header
  header: {
    title: '決定ルーレット',
    subtitle: '選択肢を追加して回転して決めましょう！',
    iconAlt: 'ルーレットアイコン'
  },

  // Input section for adding options
  input: {
    sectionTitle: '選択肢を追加',
    placeholder: '選択肢を入力...',
    addButton: '追加',
    clearAllButton: 'すべてクリア',
    maxOptionsReached: '最大制限の {{max}} 個の選択肢に達しました',
    emptyOptionError: '有効な選択肢を入力してください',
    duplicateOptionError: 'この選択肢は既に存在します',
    optionTooLong: '選択肢が長すぎます（最大 {{max}} 文字）',
    defaultOptions: [
      '選択肢 1',
      '選択肢 2',
      '選択肢 3',
      '選択肢 4'
    ],
    clearAllConfirmation: '本当にすべての {{count}} 個の選択肢を削除しますか？'
  },

  // Wheel section
  wheel: {
    spinButton: 'ルーレットを回す！',
    spinningButton: '回転中...',
    noOptionsMessage: 'ルーレットを回すには最低2つの選択肢を追加してください',
    centerAlt: 'ルーレットの中心',
    pointerAlt: 'ルーレットの針'
  },

  // Result modal
  result: {
    winnerTitle: '🎉 当選！ 🎉',
    winnerPrefix: '結果は：',
    closeButton: '閉じる',
    spinAgainButton: 'もう一度回す',
    celebrationMessages: [
      'おめでとうございます！',
      '素晴らしい選択！',
      'これがあなたの答えです！',
      '完璧！',
      'すごい！'
    ]
  },

  // Language selector
  language: {
    selectorTitle: '言語',
    autoDetected: '自動検出',
    changeLanguage: '言語を {{language}} に変更',
    currentLanguage: '現在の言語：{{language}}',
    searchPlaceholder: '言語を検索...',
    mostPopularLanguages: '最も人気の言語',
    popularTag: '人気',
    tierNames: {
      mostPopular: '最も人気の言語',
      regional: '地域言語',
      europeanAsian: 'ヨーロッパ・アジア言語',
      additional: 'その他の言語'
    },
    languages: {
      en: '英語',
      es: 'スペイン語',
      fr: 'フランス語',
      pt: 'ポルトガル語',
      de: 'ドイツ語',
      zh: '中国語',
      ja: '日本語',
      ru: 'ロシア語'
    }
  },

  // Options management
  options: {
    deleteOption: '選択肢を削除：{{option}}',
    editOption: '選択肢を編集：{{option}}',
    optionNumber: '選択肢 {{number}}',
    totalOptions: '合計 {{count}} 個の選択肢',
    noOptions: '選択肢が追加されていません',
    noOptionsYet: 'まだ選択肢が追加されていません。いくつかの選択肢を追加して始めましょう！',
    noOptionsWheel: '選択肢が追加されていません',
    noOptionsWheelDesc: 'いくつかの選択肢を追加して決定ルーレットを始めましょう！',
    saveEdit: '保存',
    cancelEdit: 'キャンセル',
    editButton: '選択肢を編集',
    removeButton: '選択肢を削除',
    saveEditButton: '変更を保存',
    cancelEditButton: '編集をキャンセル'
  },

  // Wheel history
  history: {
    title: '結果履歴',
    button: '履歴',
    historyButton: '履歴',
    noHistory: '回転履歴がありません',
    lastResult: '最後の結果：{{result}}',
    clearHistory: '履歴をクリア',
    resultNumber: '結果 #{{number}}',
    timeAgo: '{{time}}前'
  },

  // Error messages
  errors: {
    loadingTranslations: '翻訳の読み込みエラー',
    wheelSpinError: 'ルーレット回転エラー',
    saveOptionsError: '選択肢の保存エラー',
    loadOptionsError: '選択肢の読み込みエラー',
    genericError: '予期しないエラーが発生しました',
    networkError: 'ネットワークエラー。インターネット接続を確認してください。',
    storageError: 'ローカルストレージへのアクセスエラー'
  },

  // Success messages
  success: {
    optionAdded: '選択肢が正常に追加されました',
    optionDeleted: '選択肢が正常に削除されました',
    optionEdited: '更新されました："{{oldText}}" → "{{newText}}"',
    optionsCleared: 'すべての選択肢がクリアされました',
    languageChanged: '言語が {{language}} に変更されました',
    historyCleared: '履歴が正常にクリアされました',
    historyLoaded: 'ルーレット設定が読み込まれました！'
  },

  // Accessibility labels
  accessibility: {
    spinButton: 'ルーレットを回すボタン',
    addOptionButton: '新しい選択肢を追加するボタン',
    deleteOptionButton: 'この選択肢を削除するボタン',
    optionInput: '新しい選択肢を入力するテキストフィールド',
    wheelCanvas: 'インタラクティブルーレットキャンバス',
    languageSelector: '言語セレクター',
    closeModal: '結果ウィンドウを閉じる',
    optionsList: '追加された選択肢のリスト'
  },

  // Time formatting
  time: {
    now: '今',
    secondsAgo: '{{count}} 秒',
    secondsAgo_plural: '{{count}} 秒',
    minutesAgo: '{{count}} 分',
    minutesAgo_plural: '{{count}} 分',
    hoursAgo: '{{count}} 時間',
    hoursAgo_plural: '{{count}} 時間',
    daysAgo: '{{count}} 日',
    daysAgo_plural: '{{count}} 日'
  },

  // Loading states
  loading: {
    loadingApp: 'アプリケーションを読み込み中...',
    loadingTranslations: '翻訳を読み込み中...',
    spinning: 'ルーレットを回転中...',
    savingOptions: '選択肢を保存中...',
    loadingOptions: '選択肢を読み込み中...'
  },

  // Tooltips and help text
  tooltips: {
    addOption: 'ルーレットに新しい選択肢を追加',
    deleteOption: 'ルーレットからこの選択肢を削除',
    clearAll: 'ルーレットからすべての選択肢を削除',
    spinWheel: 'ルーレットを回してランダムな結果を得る',
    changeLanguage: 'アプリケーションの言語を変更',
    wheelCenter: 'ルーレットの中心'
  },

  // Keyboard shortcuts
  shortcuts: {
    addOption: 'Enter キーを押して選択肢を追加',
    spinWheel: 'スペースキーを押してルーレットを回す',
    closeModal: 'Esc キーを押して閉じる'
  },

  // Validation messages
  validation: {
    required: 'このフィールドは必須です',
    minLength: '最小 {{min}} 文字',
    maxLength: '最大 {{max}} 文字',
    invalidCharacters: '無効な文字が含まれています',
    duplicateEntry: 'この項目は既に存在します'
  },

  // Social proof section
  socialProof: {
    happyUsers: '満足したユーザー',
    decisionsMade: '決定数',
    userRating: 'ユーザー評価',
    trustedText: '世界中の何千ものユーザーに信頼されています'
  },

  // Recent activity messages
  recentActivity: {
    messages: [
      'ニューヨークの誰かがルーレットを回しました',
      'ロンドンのユーザーが結果をシェアしました',
      '東京の誰かが決定を下しました',
      'シドニーのユーザーがルーレットを使用しました',
      'ベルリンの誰かが答えを得ました'
    ]
  },

  // AdSense Compliance Content
  content: {
    howToUse: {
      title: '決定ルーレットの使い方',
      step1: {
        title: 'ステップ1：選択肢を追加する',
        description: '検討したいすべての選択肢を追加することから始めましょう。入力欄に各選択肢を入力し、「追加」をクリックして決定ルーレットに含めます。必要な数だけ選択肢を追加できます - ルーレットはすべての選択肢に合わせて自動調整されます。'
      },
      step2: {
        title: 'ステップ2：ルーレットを回す',
        description: 'すべての選択肢を追加したら、「ルーレットを回す！」ボタンをクリックしてランダム選択プロセスを開始します。ルーレットは数秒間回転してから選択肢の一つに止まり、偏りのない決定を提供します。'
      },
      step3: {
        title: 'ステップ3：結果を得る',
        description: 'ルーレットが回転を止めると、選択された選択肢がハイライトされて表示されます。結果に満足しない場合は、いつでも再度回すか、好みをより良く反映するように選択肢リストを修正できます。'
      }
    },
    popularUses: {
      title: '決定ルーレットの人気の使い方',
      foodDecisions: {
        title: '🍕 食事の決定',
        description: '夕食に何を食べるか決められませんか？お気に入りのレストラン、料理、食事オプションをルーレットに追加して、選んでもらいましょう。食事場所について合意できないカップルや家族に最適です。'
      },
      entertainmentChoices: {
        title: '🎬 エンターテイメントの選択',
        description: 'スピナーを使って見る映画、プレイするゲーム、またはする活動を選びましょう。エンターテイメントオプションを追加して、ルーレットに夜の計画を決めてもらいましょう。'
      },
      travelPlanning: {
        title: '✈️ 旅行計画',
        description: '次の休暇を計画していますが、行き先を決められませんか？潜在的な旅行先をリストアップし、ルーレットを回してランダムに次の冒険を選びましょう。'
      },
      dailyActivities: {
        title: '🎯 日常活動',
        description: 'ルーレットを使ってワークアウトルーチン、週末の活動、または最初に取り組むタスクを選ぶことで、日常の決定をより楽しくしましょう。'
      },
      partyGames: {
        title: '🎪 パーティーゲーム',
        description: 'ルーレットをゲーム選択、真実ゲームやチャレンジ、参加者への楽しいタスクの割り当てに使用して、パーティーや集まりに興奮を追加しましょう。'
      },
      educationalTools: {
        title: '📚 教育ツール',
        description: '教師は決定ルーレットをランダムな生徒選択、議論トピックの選択、または教室活動をより魅力的で公平にするために使用できます。'
      }
    },
    whyChoose: {
      title: 'なぜ私たちの決定ルーレットを選ぶのか？',
      randomResults: {
        title: '🔄 真にランダムな結果',
        description: '私たちのアルゴリズムは完全に偏りのない選択を保証し、各選択肢に等しい選ばれる機会を与えます。隠れた好みやパターンはありません。'
      },
      mobileFriendly: {
        title: '📱 モバイルフレンドリー',
        description: 'すべてのデバイス - スマートフォン、タブレット、デスクトップコンピューター で完璧に動作します。レスポンシブデザインにより、画面サイズに関係なくスムーズな体験を保証します。'
      },
      noRegistration: {
        title: '🚀 登録不要',
        description: 'アカウントを作成したり個人情報を提供したりすることなく、すぐにルーレットの使用を開始できます。シンプル、高速、匿名。'
      },
      beautifulDesign: {
        title: '🎨 美しいデザイン',
        description: 'スムーズなアニメーションとグラスモーフィズム効果を備えた現代的で視覚的に魅力的なインターフェースで、意思決定をより楽しくします。'
      },
      fastPerformance: {
        title: '⚡ 高速パフォーマンス',
        description: '最小限の読み込み時間とスムーズな回転アニメーションで速度に最適化されています。イライラする遅延なしに素早く決定を下せます。'
      },
      multipleLanguages: {
        title: '🌐 複数言語対応',
        description: '世界中のユーザーにサービスを提供するために複数の言語で利用可能です。日本語、英語、スペイン語、フランス語などを切り替えてローカライズされた体験を得られます。'
      }
    },
    tips: {
      title: 'より良い意思決定のためのヒント',
      subtitle: 'ルーレット体験を最大限に活用する',
      beSpecific: {
        title: '具体的に：',
        description: '選択肢を追加するときは、できるだけ具体的にしましょう。「イタリア料理」ではなく、「マリオのピザ」や「トニーのパスタハウス」のように、より明確な結果を得るために試してみてください。'
      },
      considerWeight: {
        title: '重みを考慮：',
        description: '他の選択肢よりも魅力的な選択肢がある場合は、選ばれる可能性を高めるために複数回追加できます。'
      },
      reviewOptions: {
        title: '選択肢を確認：',
        description: '回転する前に、すべての選択肢がまだ関連性があり、あなたにとって魅力的であることを確認するためにリストを読み直しましょう。'
      },
      trustProcess: {
        title: 'プロセスを信頼：',
        description: '一度回したら、結果に従うように努めましょう。ルーレットはあなたに代わって選択を行うことで、決断麻痺を取り除くのに役立ちます。'
      },
      useBrainstorming: {
        title: 'ブレインストーミングに使用：',
        description: '創造的または予期しない選択肢を追加して、考慮していなかった新しい可能性を発見しましょう。'
      }
    }
  }
};