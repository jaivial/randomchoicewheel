var we=(v,e)=>()=>(e||v((e={exports:{}}).exports,e),e.exports);import{_ as ve,L as xe,S as Me,a as Se}from"./components-B4hNhO3t.js";var Ve=we((Ke,j)=>{(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))n(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const r of a.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&n(r)}).observe(document,{childList:!0,subtree:!0});function t(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function n(i){if(i.ep)return;i.ep=!0;const a=t(i);fetch(i.href,a)}})();class Ee{constructor(e){this.wheel=e}createEmptyWheel(){const e=document.createElement("div");e.className="wheel-fallback glass-card";const t=this.wheel.languageManager?this.wheel.languageManager.t("options.noOptionsWheel"):"No Options Added",n=this.wheel.languageManager?this.wheel.languageManager.t("options.noOptionsWheelDesc"):"Add some options to get started with your decision wheel!";e.innerHTML=`
    ...
    `,i.style.display="none",a.style.display="none",t.insertBefore(r,i),t.insertBefore(x,i),r.focus(),r.select();const T=x.querySelector(".save-edit"),I=x.querySelector(".cancel-edit"),$=()=>{const A=r.value.trim();if(!this.manager.isValidOption(A)){this.manager.helpers.showInputError(),r.focus();return}if(this.manager.options.filter((D,W)=>W!==e).includes(A)){this.manager.helpers.showDuplicateError(),r.focus();return}this.manager.options[e]=A,this.manager.renderOptions(),this.manager.saveOptions(),this.manager.notifyOptionsChanged(),this.manager.helpers.showEditSuccessFeedback(n,A)},H=()=>{r.remove(),x.remove(),i.style.display="",a.style.display=""};T.addEventListener("click",$),I.addEventListener("click",H),r.addEventListener("keydown",A=>{A.key==="Enter"?(A.preventDefault(),$()):A.key==="Escape"&&(A.preventDefault(),H())});const R=A=>{t.contains(A.target)||(H(),document.removeEventListener("click",R))};setTimeout(()=>{document.addEventListener("click",R)},100)}async clearAllOptions(){if(this.manager.options.length!==0){if(this.manager.options.length>1){const e=this.manager.t("input.clearAllButton"),t=this.manager.t("input.clearAllConfirmation",{count:this.manager.options.length}),n=this.manager.t("input.clearAllButton"),i=this.manager.t("options.cancelEdit");if(!await this.manager.confirmationModal.show({title:e,message:t,confirmText:n,cancelText:i}))return}this.manager.options=[],this.manager.renderOptions(),this.manager.saveOptions(),this.manager.notifyOptionsChanged(),this.manager.helpers.showClearFeedback()}}}class Oe{constructor(e=null){this.languageManager=e,this.modal=null,this.isOpen=!1,this.currentResolve=null,this.createModal(),this.setupEventListeners()}createModal(){const e=document.createElement("div");e.id="confirmation-modal",e.className="confirmation-modal hidden",e.innerHTML=`
